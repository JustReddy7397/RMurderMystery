...
public class YamlConfig {
    ...
    public YamlConfig(final String name)
        ...
        final File configFile = new File(MurderMystery.getPlugin(MurderMystery.class).getDataFolder(), completeName);

        if (!configFile.exists()) {
            //noinspection ResultOfMethodCallIgnored
            configFile.getParentFile().mkdirs();
            MurderMystery.getPlugin(MurderMystery.class).saveResource(completeName, false);
        }
